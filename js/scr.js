(function () {
    ...
    if (typeof nokia === "undefined"
            || typeof nokia.places === "undefined"
            || typeof nokia.maps === "undefined") {
        alert("Error! Nokia libraries not loaded!");
        return;
    }
    var hereApp = angular.module('here', []);

    hereApp.controller("RoutePlannerController", function () {
        ...
        routeCtrl.moveUp = function (dest) {
            var idx = routeCtrl.waypoints.indexOf(dest),
                waypoint = routeCtrl.waypoints.splice(idx, 1)[0];
            ...
            new nokia.places.widgets.SearchBox({
                ...
                onResults : function (data) {
                    if (data.results) {
                        ...
                        // fix places data
                        searchCtrl.results.forEach(function (waypoint) {
                            if (waypoint.vicinity) {
                                waypoint.vicinity = waypoint.vicinity.replace("<br/>", ",");
                            }
                        });
                    } else {
                        searchCtrl.results = [];
